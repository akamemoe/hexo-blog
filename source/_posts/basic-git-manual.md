...
# 指定克隆后的文件夹名称
git clone https://github.com/username/repo.git git-study # 如果后面是 . 在当前目录创建
...
git clone --depth=1 --no-single-branch https://github.com/username/repo.git # --no-single-branch 同时克隆其他所有分支
...
git remote add example https://github.com/username/repo.git
...
<img src="/arbitrary/media/gitrebase-3.png" width="700" />


2、除了第一条后面全部改成 `s` 或 `squash`:

<img src="/arbitrary/media/gitrebase-4.png" width="700" />


3、按 `:wq` 退出交互式，接着进入另一个交互式来编辑commit消息, 如果不需要修改之前的commit消息则直接退出：

<img src="/arbitrary/media/gitrebase-5.png" width="700" />
...
<img src="/arbitrary/media/gitrebase-1.png" width="700" />
<img src="/arbitrary/media/gitrebase-2.png" width="700" />
...
```bash
git submodule add https://github.com/username/repo.git # 默认添加到当前目录下
git submodule add https://github.com/username/repo.git submodules/subrepo  # 添加到指定目录

# -b 指定需要添加仓库的某个分支
git submodule add -b develop https://github.com/username/repo.git
```
...
```bash
git subtree add --prefix=sub/common https://github.com/username/repo.git main --squash
```
...
```bash
git subtree pull --prefix=sub/common https://github.com/username/repo.git main --squash
```
...
git commit -m "子仓库修改"
# 然后推送
git subtree push --prefix=sub/common https://github.com/username/repo.git main --squash
```
...
git log -n 1 --pretty=format:"%ae" # username@gmail.com
...
旧仓库A: https://github.com/username/A.git
新仓库B: https://github.com/username/B.git

1、克隆旧裸仓库
```bash
# 克隆裸仓库，里面没有工作区内容
git clone --bare https://github.com/username/A.git
```

2、镜像推送至新仓库
```bash
cd A
git push --mirror https://github.com/username/B.git
```
...
4、拉取新仓库
```bash
git clone https://github.com/username/B.git
```
